...
type Props = {
  ...
  onLeadEditado: (lead: Record<string, any>) => void
  onClose: () => void
  tiposSessao: TipoSessao[]
}

export default function ModalEditarLead({ fotografoId, etapas, lead, onLeadEditado, onClose, tiposSessao }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    whatsapp_cliente: lead.whatsapp_cliente || '',
    email_cliente: lead.email_cliente || '',
    tipo_servico: lead.pacote_id || (pacotes.find(p => p.nome === lead.tipo_servico)?.id) || '',
    data_pretendida: lead.data_pretendida ? new Date(lead.data_pretendida).toISOString().split('T')[0] : '',
  ...
  const [respostas, setRespostas] = useState<Record<string, any>>(lead.respostas_formulario || {})
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (errUpdate) {
      setError('Erro ao atualizar orçamento: ' + errUpdate.message)
      setSaving(false)
    } else {
      onLeadEditado(data)
      onClose()
      ...
      <div className="animate-fade-in" style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              <h2 style={{ fontSize: '1.05rem', fontWeight: 600, margin: 0 }}>Editar Orçamento</h2>
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', padding: '16px 24px', borderTop: '1px solid var(--color-border)', background: 'var(--color-bg-surface)' }}>
            ...
            <button type="submit" className="btn btn-primary" style={{ flex: 2 }} disabled={saving}>
              {saving ? <><Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> Salvando...</> : <><Save size={14} /> Salvar Orçamento</>}
            </button>
          </div>
        </form>
      </div>
