...
export default function ServicosClient({ fotografoId, servicosIniciais, etapasIniciais = [] }: Props) {
  ...
  function handleOpenModal(service: Service | null = null) {
    ...
    if (service) {
      ...
      setFormFields([])
      setUsarFunilExclusivo(false)
      setFormEtapas(etapas.filter(e => !e.pacote_id).sort((a,b) => a.ordem - b.ordem)) // Carrega o padrão como base
    }
    setShowModal(true)
  }
  ...
  async function handleSave(e: React.FormEvent) {
    e.preventDefault()
    if (!formName.trim()) { setError('O nome do serviço é obrigatório.'); return }
    setSaving(true)
    setError('')
    ...
    try {
      ...
      if (usarFunilExclusivo && formEtapas.length > 0) {
  ...
  async function handleDelete(id: string) {
    if (!confirm('Deseja realmente excluir permanentemente? Isso removerá custos, formulários, funis e vínculos de pacotes associados.')) return
    setDeletingId(id)
    try {
    ...
    <>
      <div className="topbar">
        ...
        <div className="topbar-actions">
          <button onClick={() => handleOpenModal()} className="btn btn-primary btn-sm"><Plus size={15} /> Novo Item</button>
        </div>
      </div>
      ...
      <div className="page-content animate-fade-in">
        ...
        <div className="page-header">
          <h1 style={{ fontSize: '1.25rem', marginBottom: '4px' }}>Serviços e Pacotes</h1>
          <p style={{ fontSize: '0.875rem', color: 'var(--color-text-muted)' }}>Configure serviços, pacotes, formulários e jornadas exclusivas de atendimento.</p>
        </div>
        ...
        {servicos.length === 0 ? (
          <div className="card" style={{ textAlign: 'center', padding: '56px 24px', border: '1px dashed var(--color-border)' }}>
            ...
            <p style={{ marginBottom: '24px', color: 'var(--color-text-muted)', maxWidth: '420px', margin: '0 auto 24px' }}>Crie serviços simples ou monte combos completos com formulários e jornadas próprias.</p>
            <button onClick={() => handleOpenModal()} className="btn btn-primary"><Plus size={14} /> Começar Catálogo</button>
          </div>
        ) : (
          ...
          <div className="animate-fade-in" style={{ background: 'var(--color-bg-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '780px', maxHeight: '94vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.4)' }}>
            <div style={{ padding: '18px 24px', borderBottom: '1px solid var(--color-border)', background: formColor ? `linear-gradient(135deg, ${formColor}14 0%, transparent 60%)` : undefined, borderRadius: 'var(--radius-xl) var(--radius-xl) 0 0' }}>
              ...
              <div style={{ display: 'flex', gap: '16px', marginTop: '20px', borderBottom: '1px solid var(--color-border)', margin: '20px -24px -18px', padding: '0 24px', overflowX: 'auto' }}>
                ...
                <TabButton active={activeTab === 'funil'} onClick={() => setActiveTab('funil')} icon={<Kanban size={14}/>} label="Funil & Jornada" />
              ...
              <div style={{ overflowY: 'auto', flex: 1, padding: '24px', display: 'flex', flexDirection: 'column', gap: '28px' }}>
                  ...
                  <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    {!editingService && (
                      <div style={{ display: 'flex', gap: '10px', background: 'var(--color-bg-base)', padding: '6px', borderRadius: 'var(--radius-lg)' }}>
                        <button type="button" onClick={() => setFormIsPacote(false)} className={`btn ${!formIsPacote ? 'btn-primary' : 'btn-ghost'}`} style={{ flex: 1 }}><Tag size={16} /> Serviço Individual</button>
                        <button type="button" onClick={() => setFormIsPacote(true)} className={`btn ${formIsPacote ? 'btn-primary' : 'btn-ghost'}`} style={{ flex: 1 }}><Package size={16} /> Pacote / Combo</button>
                      </div>
                    )}
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px' }}>
                      <div className="form-group" style={{ margin: 0 }}><label className="form-label">Nome *</label><input type="text" className="form-input" required placeholder={formIsPacote ? "Ex: Combo Maternidade" : "Ex: Ensaio Gestante"} value={formName} onChange={e => setFormName(e.target.value)} /></div>
                ...
                {activeTab === 'itens' && formIsPacote && (
                  <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    <div className="alert alert-info" style={{ fontSize: '0.8125rem', display: 'flex', gap: '10px' }}><Layers size={18} style={{ flexShrink: 0 }} /><span>Selecione quais serviços individuais estão inclusos neste pacote.</span></div>
                ...
                {activeTab === 'formulario' && (
                  <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <p style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)', margin: 0, maxWidth: '400px' }}>Crie campos dinâmicos para o Orçamento deste {formIsPacote ? 'Pacote' : 'Serviço'}.</p>
                      <button type="button" onClick={addFormField} className="btn btn-secondary btn-sm"><Plus size={14} /> Novo Campo</button>
                    </div>
                    {formFields.length === 0 ? (
                       <div style={{ textAlign: 'center', padding: '40px', border: '1px dashed var(--color-border)', borderRadius: 'var(--radius-md)', color: 'var(--color-text-muted)' }}>Nenhum campo personalizado.</div>
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', background: 'var(--color-accent-subtle)', borderRadius: 'var(--radius-md)', border: `1px solid var(--color-accent)` }}>
                      <div>
                        ...
                        <p style={{ margin: 0, fontSize: '0.75rem', color: 'var(--color-text-secondary)', maxWidth: '400px' }}>
                          Ative se este {formIsPacote ? 'pacote' : 'serviço'} precisar de uma jornada ou esteira de produção diferente do padrão do estúdio.
                        </p>
                      </div>
                ...
                {activeTab === 'custos' && (
                  <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <p style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)', margin: 0, maxWidth: '400px' }}>Defina os custos diretos da entrega. Isso gera sua Margem de Lucro real.</p>
                    ...
                    <div style={{ marginTop: '10px', background: 'var(--color-bg-base)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: '16px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div><span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', textTransform: 'uppercase' }}>Custo Total</span><div style={{ fontSize: '1.1rem', fontWeight: 600 }}>{formatCurrency(totalCost)}</div></div>
                        <div style={{ textAlign: 'right' }}><span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', textTransform: 'uppercase' }}>Lucro Líquido Previsto</span><div style={{ fontSize: '1.25rem', fontWeight: 800, color: getMarginColor(profitMargin) }}>{formatCurrency(estimatedProfit)}</div></div>
                      </div>
                    </div>
                  </div>
                )}
